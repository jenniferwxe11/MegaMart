...
def missing_channel(df, idx, ctx):

    df.at[idx, "channel"] = None


def missing_assignment_group(df, idx, ctx):

    df.at[idx, "assignment_group"] = None


def missing_eligible(df, idx, ctx):

    df.at[idx, "eligible"] = pd.NA


def missing_exposed(df, idx, ctx):

    df.at[idx, "exposed"] = pd.NA


def missing_exposed_time(df, idx, ctx):

    df.at[idx, "exposed_time"] = None


def missing_opened(df, idx, ctx):

    df.at[idx, "opened"] = pd.NA


def missing_opened_time(df, idx, ctx):

    df.at[idx, "opened_time"] = None


def missing_clicked(df, idx, ctx):

    df.at[idx, "clicked"] = pd.NA


def missing_clicked_time(df, idx, ctx):

    df.at[idx, "clicked_time"] = None


def missing_device_platform(df, idx, ctx):

    df.at[idx, "device_platform"] = None


def missing_cost_per_msg(df, idx, ctx):
...
def invalid_channel(df, idx, ctx):
    """
...
def invalid_assignment_group(df, idx, ctx):
    """
    assignment_group must be one of:
        Treatment
        Control
    """
...
def invalid_device_platform(df, idx, ctx):
    """
    device_platform must be one of:
        iOS
        Android
        Web
    """
...
def cost_per_msg_out_of_range(df, idx, ctx):
    """
...
def duplicate_campaign_exposure(df, idx, ctx):
    """
    A campaign exposure must only appear once per:
        customer_id
        campaign_id
        channel

    This rule creates a duplicate combination.
    """
    ...
    mask = (
        ...
        & (df["campaign_id"] == row["campaign_id"])
        & (df["channel"] == row["channel"])
    )

    existing_indices = df.index[mask].tolist()

    if len(existing_indices) <= 1:
        return

    duplicate_idx = random.choice([i for i in existing_indices if i != idx])

    df.at[idx, "customer_id"] = df.at[duplicate_idx, "customer_id"]
    df.at[idx, "campaign_id"] = df.at[duplicate_idx, "campaign_id"]
    df.at[idx, "channel"] = df.at[duplicate_idx, "channel"]
...
def channel_not_in_campaign_channels(df, idx, ctx):
    ...
    invalid_channels = [
        value for value in invalid_channels if value not in campaign_channels
    ]
...
def control_group_exposure_violation(df, idx, ctx):
    """
...
def eligible_must_be_true(df, idx, ctx):
    """
...
def invalid_cost_per_msg(df, idx, ctx):
    """
...
def exposed_without_exposed_time(df, idx, ctx):
    """
...
def exposed_time_without_exposure(df, idx, ctx):
    """
...
def opened_without_exposure(df, idx, ctx):
    """
...
def clicked_without_open(df, idx, ctx):
    """
...
def opened_on_exposure_only_channel(df, idx, ctx):
    """
...
def opened_without_opened_time(df, idx, ctx):
    """
...
def opened_time_before_exposed_time(df, idx, ctx):
    """
...
def clicked_without_clicked_time(df, idx, ctx):
    """
...
def clicked_time_before_opened_time(df, idx, ctx):
    """
...
def clicked_time_before_exposed_time(df, idx, ctx):
    """
...
def future_exposed_time(df, idx, ctx):
    """
    exposed_time must not be in the future.
    """
...
def future_opened_time(df, idx, ctx):
    """
    opened_time must not be in the future.
    """
...
def future_clicked_time(df, idx, ctx):
    """
    clicked_time must not be in the future.
    """
