...
def dirty_product_content(ctx: GenerationContext):
    ...
    append_error(
        df,
        no_img_idx,
        error_label="has_image=False but image_count > 0",
        columns=["has_image", "image_count"],
    )
    ...
    append_error(
        df,
        qs_idx,
        error_label="image_quality_score > 1.0",
    ...
    append_error(
        df,
        dl_idx,
        error_label="negative description length",
    ...
    append_error(
        df,
        mac_idx,
        error_label="negative missing attribute count",
        columns=["missing_attribute_count"],
    )
