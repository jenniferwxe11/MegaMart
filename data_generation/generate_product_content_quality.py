...
product_lifecycles_df = pd.read_csv(
    "data_generation/raw_data/product_lifecycles_raw.csv"
)
...
category_attributes_df = pd.read_csv(
    "data_generation/raw_data/category_attributes_raw.csv"
)

product_content_quality = []

for product_id in product_ids:
    product_lifecycle = product_lifecycles_df.loc[
        product_lifecycles_df["product_id"] == product_id, "status"
    ].iloc[0]
    ...
    category_attributes = (
        category_attributes_df.loc[
            category_attributes_df["category"] == category, "attribute"
        ]
        .dropna()
        .tolist()
    )
    ...
    if product_lifecycle == "New":
        ...
        quality_tier = random.choices(
            ["poor", "average", "good", "excellent"], weights=[0.2, 0.4, 0.3, 0.1], k=1
        )[0]
    ...
    launch_date = product_lifecycles_df.loc[
        product_lifecycles_df["product_id"] == product_id, "launch_date"
    ].iloc[0]
    launch_date = launch_date.date()
    discontinuation_date = product_lifecycles_df.loc[
        product_lifecycles_df["product_id"] == product_id, "discontinuation_date"
    ].iloc[0]
    ...
    if recent_bias_years is not None:
        biased_start = max(launch_date, date(end_date.year - recent_bias_years, 1, 1))
        last_updated = fake.date_between(start_date=biased_start, end_date=end_date)
...
df_product_content_quality.to_csv(
    "data_generation/raw_data/product_content_quality_raw.csv", index=False
)
